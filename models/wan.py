...
# Fix the broken WanLayerNorm
def fixed_wan_layernorm_forward(self, x):
    if hasattr(self, 'weight') and self.weight is not None:
        target_dtype = self.weight.dtype
    else:
        target_dtype = x.dtype  # Use input dtype if no weight
    return super(type(self), self).forward(x.to(target_dtype)).type_as(x)

# Apply the fix
from wan.modules.model import WanLayerNorm
WanLayerNorm.forward = fixed_wan_layernorm_forward
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            # print(f"\n--- cross_attn_ffn DEBUG ---")
            # debug_tensor_info(x, "ffn_input_x")
            # debug_tensor_info(context, "ffn_context")
            # debug_tensor_info(e[3], "ffn_e3")
            # debug_tensor_info(e[4], "ffn_e4")

            # Cross attention
            x = x + self.cross_attn(self.norm3(x), context, context_lens)
            # debug_tensor_info(x, "after_cross_attn")

            # Before FFN
            norm2_output = self.norm2(x)
            # debug_tensor_info(norm2_output, "norm2_output")

            modulated_input = norm2_output * (1 + e[4]) + e[3]
            # debug_tensor_info(modulated_input, "modulated_ffn_input")

            # Check FFN layer info
            if hasattr(self.ffn, '__getitem__'):
                first_layer = self.ffn[0]
            #     print(f"FFN first layer: in_features={first_layer.in_features}")
            #     print(f"FFN weight shape: {first_layer.weight.shape}")
            #     print(f"FFN weight dtype: {first_layer.weight.dtype}")
            #     print(f"FFN weight device: {first_layer.weight.device}")
            #
            # print(f"About to call FFN with input shape: {modulated_input.shape}")

            y = self.ffn(modulated_input)  # This is where the error occurs
            # debug_tensor_info(y, "ffn_output")

            x = x + y * e[5]
            # print(f"--- cross_attn_ffn END ---\n")
            return x
...
class InitialLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.freqs.device != self.patch_embedding.weight.device:
        ...
        if self.patch_embedding.weight.device != x[0].device:
            self.patch_embedding = self.patch_embedding.to(x[0].device)

        # ── Device synchronization ─────────────────────────────────────
        tgt = x[0].device

        # Move all components to the target device if needed
        if self.freqs.device != tgt:
            self.freqs = self.freqs.to(tgt)

        if self.patch_embedding.weight.device != tgt:
            self.patch_embedding = self.patch_embedding.to(tgt)

        if next(self.text_embedding.parameters()).device != tgt:
            self.text_embedding = self.text_embedding.to(tgt)

        if next(self.time_embedding.parameters()).device != tgt:
            self.time_embedding = self.time_embedding.to(tgt)

        if next(self.time_projection.parameters()).device != tgt:
            self.time_projection = self.time_projection.to(tgt)

        if hasattr(self, 'img_emb') and next(self.img_emb.parameters()).device != tgt:
            self.img_emb = self.img_emb.to(tgt)
        # ──────────────────────────────────────────────────────────────

        # print(f"text_embedding type: {type(self.text_embedding)}")
        # print(f"text_embedding: {self.text_embedding}")
...
# def debug_tensor_info(tensor, name, detailed=False):
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print(f"\n=== TransformerLayer {self.block_idx} ENTRY ===")
        # debug_tensor_info(x, "x")
        # debug_tensor_info(e, "e")
        # debug_tensor_info(e0, "e0")
        # debug_tensor_info(seq_lens, "seq_lens")
        # debug_tensor_info(grid_sizes, "grid_sizes")
        # debug_tensor_info(freqs, "freqs")
        # debug_tensor_info(context, "context")

        tgt = x.device
        block_device = next(self.block.parameters()).device

        # print(f"Target device: {tgt}, Block device: {block_device}")
        # Check if this is a carousel-managed layer

        # if block_device != tgt:
        #     print(f"🚨 TRANSFER TRIGGERED - Layer {self.block_idx}")
        #     print(f"   Input tensor device: {tgt}")
        #     print(f"   Block parameters device: {block_device}")
        #     print(f"   This should NOT happen if carousel is working correctly!")
        #     if block_device.type == 'cpu' and tgt.type == 'cuda':
        #         # print(f"=== MOVING TO CPU (Block {self.block_idx}) ===")
        #
        #         # Store original dtypes for restoration
        #         original_dtypes = {}
        #
        #         def convert_float8_to_bfloat16(module, prefix=""):
        #             """Convert float8_e4m3fn parameters and buffers to bfloat16 for computation"""
        #             converted_count = 0
        #
        #             for name, param in module.named_parameters(recurse=True):  # Add recurse=True
        #                 full_name = f"{prefix}.{name}" if prefix else name
        #                 if param.dtype == torch.float8_e4m3fn:
        #                     original_dtypes[('param', full_name)] = param.dtype
        #                     param.data = param.data.to(torch.bfloat16)
        #                     converted_count += 1
        #
        #             for name, buffer in module.named_buffers(recurse=True):  # Add recurse=True
        #                 full_name = f"{prefix}.{name}" if prefix else name
        #                 if buffer.dtype == torch.float8_e4m3fn:
        #                     original_dtypes[('buffer', full_name)] = buffer.dtype
        #                     buffer.data = buffer.data.to(torch.bfloat16)
        #                     converted_count += 1
        #
        #             return converted_count
        #
        #         # Convert float8 weights to bfloat16 for computation
        #         converted_count = convert_float8_to_bfloat16(self.block)
        #         # print(f"Converted {converted_count} parameters/buffers from float8_e4m3fn to bfloat16")
        #
        #         # Move inputs to CPU - keep them in bfloat16
        #         x_cpu = x.to('cpu', dtype=torch.bfloat16)
        #         e0_cpu = e0.to('cpu', dtype=torch.bfloat16)
        #         seq_lens_cpu = seq_lens.to('cpu')
        #         grid_sizes_cpu = grid_sizes.to('cpu')
        #         freqs_cpu = freqs.to('cpu')
        #         context_cpu = context.to('cpu', dtype=torch.bfloat16)
        #
        #         # print("=== INPUTS MOVED TO CPU ===")
        #         # debug_tensor_info(x_cpu, "x_cpu")
        #         # debug_tensor_info(e0_cpu, "e0_cpu")
        #         # debug_tensor_info(context_cpu, "context_cpu")
        #
        #         # Verify all tensors and weights are now bfloat16
        #         def check_dtypes(module, prefix=""):
        #             """Verify all parameters are bfloat16 before computation"""
        #             for name, param in module.named_parameters(recurse=False):
        #                 full_name = f"{prefix}.{name}" if prefix else name
        #                 if param.dtype not in [torch.bfloat16, torch.float32]:
        #                     print(f"WARNING: Parameter {full_name} has unexpected dtype {param.dtype}")
        #
        #             for name, child in module.named_children():
        #                 child_prefix = f"{prefix}.{name}" if prefix else name
        #                 check_dtypes(child, child_prefix)
        #
        #         check_dtypes(self.block)
        #
        #         # print("=== RUNNING BLOCK ON CPU ===")
        #         # Run computation with autocast disabled to avoid dtype issues
        #         with torch.autocast('cpu', enabled=False):
        #             try:
        #                 x_result = self.block(x_cpu, e0_cpu, seq_lens_cpu, grid_sizes_cpu, freqs_cpu, context_cpu, None)
        #             except RuntimeError as e:
        #                 print(f"ERROR during CPU computation: {e}")
        #                 # Additional debugging
        #                 print(f"x_cpu dtype: {x_cpu.dtype}")
        #                 print(f"e0_cpu dtype: {e0_cpu.dtype}")
        #                 print(f"context_cpu dtype: {context_cpu.dtype}")
        #
        #                 # Check first linear layer in block
        #                 if hasattr(self.block, 'ffn') and hasattr(self.block.ffn, '__getitem__'):
        #                     first_layer = self.block.ffn[0]
        #                     print(f"FFN first layer weight dtype: {first_layer.weight.dtype}")
        #
        #                 raise e
        #
        #         # print("=== BLOCK OUTPUT ===")
        #         # debug_tensor_info(x_result, "x_result")
        #
        #         # Restore original dtypes (convert back to float8_e4m3fn)
        #         def restore_original_dtypes(module, prefix=""):
        #             restored_count = 0
        #
        #             for name, param in module.named_parameters(recurse=True):  # Add recurse=True
        #                 full_name = f"{prefix}.{name}" if prefix else name
        ...
        #                     param.data = param.data.to(original_dtypes[key])
        #                     restored_count += 1
        #
        #             for name, buffer in module.named_buffers(recurse=True):  # Add recurse=True
        #                 full_name = f"{prefix}.{name}" if prefix else name
        #                 key = ('buffer', full_name)
        #                 if key in original_dtypes:
        #                     buffer.data = buffer.data.to(original_dtypes[key])
        #                     restored_count += 1
        #
        #             return restored_count
        #
        #         restored_count = restore_original_dtypes(self.block)
        #         # print(f"Restored {restored_count} parameters/buffers to original dtypes")
        #
        #         if self.block_idx % 2 == 0:  # Every 3 layers
        #             import gc
        #             torch.cuda.empty_cache() if torch.cuda.is_available() else None
        #             gc.collect()
        #
        #         # Move result back to GPU, ensuring it matches the original input dtype
        #         x = x_result.to(tgt, dtype=x.dtype)
        #         # print("=== MOVED BACK TO GPU ===")
        #         # debug_tensor_info(x, "x_final")
        #     else:
        #         # Cross-GPU transfer (e.g., GPU 1 → GPU 0)
        #         print(f"=== CROSS-GPU TRANSFER (Block {self.block_idx}) ===")
        #         print(f"Moving from {block_device} to {tgt}")
        #         # Simple device transfer for GPU-to-GPU
        #         # Move inputs to block's device
        #         x_gpu = x.to(block_device)
        #         e0_gpu = e0.to(block_device)
        #         seq_lens_gpu = seq_lens.to(block_device)
        #         grid_sizes_gpu = grid_sizes.to(block_device)
        #         freqs_gpu = freqs.to(block_device)
        #         context_gpu = context.to(block_device)
        #
        #         # Run computation on block's GPU
        #         x_result = self.block(x_gpu, e0_gpu, seq_lens_gpu, grid_sizes_gpu, freqs_gpu, context_gpu, None)
        #
        #         # Move result back to target GPU
        #         x = x_result.to(tgt)
        #
        # else:
        #     # print(f"=== RUNNING ON GPU (Block {self.block_idx}) ===")
        #     # Same device - no transfer needed
        #     self.offloader.wait_for_block(self.block_idx)
        #     x = self.block(x, e0, seq_lens, grid_sizes, freqs, context, None)
        #     self.offloader.submit_move_blocks_forward(self.block_idx)
        #     # print("=== GPU BLOCK OUTPUT ===")
        #     # debug_tensor_info(x, "x_gpu_result")

        self.offloader.wait_for_block(self.block_idx)
        x = self.block(x, e0, seq_lens, grid_sizes, freqs, context, None)
        self.offloader.submit_move_blocks_forward(self.block_idx)

        # print(f"=== TransformerLayer {self.block_idx} EXIT ===\n")
